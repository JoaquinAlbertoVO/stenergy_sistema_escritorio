...
// Inscribir a un usuario en un curso (Usa nuestro endpoint personalizado en WP)
export const enrollStudent = async (email, name, courseId, dni) => {
  try {
    ...
    return { success: false, error: error.response?.data?.message || 'Error en la inscripción a TutorLMS' };
  }
};

// Enlazar el certificado ya generado en el servidor al perfil del alumno en WordPress
// No sube el archivo (ya existe en stenergyedu.com/CERTIFICADOS_2026/), solo guarda la URL
export const linkCertificateToWP = async (publicUrl, courseId, dni) => {
  try {
    ...
    return { success: false, error: error.response?.data?.message || 'Error enlazando certificado a WP' };
  }
};
